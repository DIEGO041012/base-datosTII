...
<body>
    ...
    <form class="w-full max-w-lg">
      ...
      <div class="flex flex-wrap -mx-3 mb-6">
        <div class="w-full px-3">
          ...
          <button class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded" type="button" onclick="editarRegistro()">
            Editar
          </button>
          <button class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" type="button" onclick="eliminarRegistro()">
            Eliminar
          </button>
        </div>
      </div>
  ...
  <script>
    ...
    function editarRegistro() {
      // Lógica para cargar los datos del registro a editar
      // por ejemplo, puedes obtener los valores de la base de datos
      var dependencia = document.getElementById('grid-dependencia').value;
      var nombre = document.getElementById('grid-nombre').value;
      var prioridad = document.getElementById('grid-prioridad').value;
      var fechaTentativa = document.getElementById('grid-fecha-tentativa').value;
  
      // Asignar los valores a los campos del formulario
      document.getElementById('grid-dependencia').value = dependencia;
      document.getElementById('grid-nombre').value = nombre;
      document.getElementById('grid-prioridad').value = prioridad;
      document.getElementById('grid-fecha-tentativa').value = fechaTentativa;
    }
